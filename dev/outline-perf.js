#!/usr/bin/env node
/* Outline interaction-performance suite: drives a LIVE app with Playwright
 * and holds every gesture to its render budget.
 *
 *   node dev/outline-perf.js [url] [--mutate]
 *
 ...
 *
 * Companion plan: dev/outline-perf-plan.md. Needs a Chromium at $CHROME or
...
(async () => {
  ...
  await page.goto(URL);
  await page.waitForSelector('.outline .md-stackhead',
    { state: 'attached', timeout: 120000 });
  await page.waitForTimeout(2500);
  const tab = await page.$('.dv-tab.dv-inactive-tab:has-text("Outline")');
  ...
  const $head = (name) => page.locator('.outline .md-stackhead', {
    has: page.locator('.md-name', { hasText: name })
  }).first();
  const clickFocusBtn = (name) => page.evaluate((nm) => {
    const h = [...document.querySelectorAll('.outline .md-stackhead')]
  ...
  await act('hover a row (lineage dim)', 0, async () => {
    const row = page.locator('.outline .md-chip[data-id]').nth(5);
    await row.hover();
  ...
  await act('ctrl-click select 3 rows', 0, async () => {
    for (let i = 2; i <= 4; i++) {
      await page.locator('.outline .md-chip[data-id]').nth(i)
        .click({ modifiers: ['Control'] });
    }
  }, { settle: 300 });
  await act('clear selection (bar)', 0, () =>
    page.evaluate(() => {
      const b2 = document.querySelector('.outline .md-actionbar .md-no');
      if (b2) b2.click();
    }), { settle: 300 });
  ...
  // Fold.
  await act('fold all', 1, () => page.click('.outline .md-fold'));
  await act('unfold all', 1, () => page.click('.outline .md-fold'));
  await act('collapse one (chevron)', 1, () => page.evaluate(() => {
    document.querySelector('.outline .md-stackhead .md-chev').click();
  }));
  await act('expand one (chevron)', 1, () => page.evaluate(() => {
    document.querySelector('.outline .md-stackchip .md-chev').click();
  }));

  // ToC pick: fold-all -> dblclick a collapsed row = focus AND fold-clear;
  // one rebuild for the fold, one for the focus.
  await act('fold all (again)', 1, () => page.click('.outline .md-fold'));
  await act('ToC dblclick -> focus', 1, () =>
    page.locator('.outline .md-stackchip').first()
      .dblclick({ position: { x: 200, y: 12 } }));
  await act('exit -> full list', 1, () => page.click('.outline .md-crumb-x'));
  ...
  await act('row click (reveal panel)', [0, 2], () =>
    page.locator('.outline .md-chip[data-id]').nth(3).click(),
  ...
  for (let i = 0; i < 30; i++) {
    ...
    await page.waitForSelector('.outline .md-crumb-cur', { timeout: 5000 });
    await page.click('.outline .md-crumb-x');
    await page.waitForSelector('.outline .md-crumb[hidden]',
  ...
  if (MUTATE) {
    await act('append while focused (MUTATES)', [1, 2], async () => {
      ...
      const row = page.locator('.outline .md-chip.instack').nth(2);
      await row.hover();
      await row.locator('.md-rowadd').click();
      await page.waitForSelector('.outline .md-pick-row', { timeout: 8000 });
      await page.evaluate(() =>
        document.querySelector('.outline .md-pick-row').click());
